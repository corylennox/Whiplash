...
class PlatformGenerator: SKSpriteNode
{
    ...
    var descentSpeed:CGFloat = PLATFORM_DESCENT_SPEED
    ...
    {
        let platform = Platform(newRotationSpeed: rotationSpeed, newLateralSpeed: lateralSpeed!, newDownSpeed: descentSpeed, color: getColor())
    ...
    {
        if(score < 35){
            rotationSpeed = pow(0.018 * CGFloat(score)+0.15, 2)*5 + 1.7
        }
    ...
    func updateLongitudinalSpeed(score: Int)
    {
            let factor:CGFloat = 8
            descentSpeed = PLATFORM_DESCENT_SPEED + factor * CGFloat(score)
            for platform in platforms {
                platform.updateDescentSpeed(moveBySpeed: factor)
        }
    }
}
